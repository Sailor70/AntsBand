...
def get_test_mean(params: dict):
    ...
    for i in range(10):
        ...
        similar_notes_factor, similar_times_factor = calculate_similarity(midi_result, tracks_data[0], MidiFile(
            '../../data/theDreamingAnt.mid', clip=True))
        similar_notes_factor2, similar_times_factor2 = calculate_similarity(midi_result, tracks_data[1], MidiFile(
            '../../data/theDreamingAnt.mid', clip=True))
...
def get_tests_results():
    ...
    df.to_csv('../../data/results/results_2lines/antsBand_test_2lines2.csv', index=False)  # bez kolumny indexu
